...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'save_bescheid') {
    $formLink  = trim($_POST['bh_link'] ?? '');
    $formDatum = trim($_POST['bh_date'] ?? '');
    $formNr    = trim($_POST['bh_belegnr'] ?? '');
    $refs      = $_POST['pos_ref'] ?? [];
    ...
    if (!$errors) {
        ...
        foreach ($positions as $pos) {
            ...
            elseif (($rec['s_bh'] ?? '') !== 'eingereicht') $errors[] = 'Beleg ' . htmlspecialchars($rec['intern_nr']) . ' ist nicht mehr im Status „eingereicht“.';
        }
    }
...
<head>
    ...
    <title>Beihilfebescheid erfassen</title>
    ...
    <style>
        ...
        .pos-row { display: flex; gap: 10px; align-items: flex-end; margin-bottom: 10px; padding: 12px; background: #fff9f0; border: 1px solid #eee; border-radius: 8px; }
...
<header class="main-header">
    ...
    <div class="header-title-center"><h1>Beihilfebescheid erfassen</h1></div>
    ...
    <form method="POST" id="bescheidForm">
        ...
        <div class="form-row">
            <div class="field" style="flex:2; min-width:260px;">
                ...
                <input type="url" name="bh_link" value="<?= htmlspecialchars($formLink) ?>" required>
            </div>
            <div class="field" style="width:150px;">
                <label>Bescheid-Datum</label>
                <input type="date" name="bh_date" value="<?= htmlspecialchars($formDatum) ?>" required>
            </div>
            <div class="field" style="width:180px;">
                <label>Bescheidnummer</label>
                <input type="text" name="bh_belegnr" value="<?= htmlspecialchars($formNr) ?>" required>
            </div>
        </div>

        <h2>Positionen</h2>
        <?php if (!$offen): ?>
            <div class="msg err">Es gibt keine eingereichten, noch offenen Beihilfe-Belege.</div>
        <?php endif; ?>
...
function setPlaceholder(sel, inp) {
    ...
    inp.placeholder = o ? 'erwartet ' + o.e_bh.toFixed(2) : '';
    const untouched = inp.value === '' || inp.value === inp.dataset.auto;
    if (untouched) {
        const v = o ? o.e_bh.toFixed(2) : '';
        inp.value = v;
        inp.dataset.auto = v;
    }
}
